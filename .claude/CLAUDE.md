...
- 関数を2個書いたら、その都度 `git commit -m` を行う
...
- コードを書く際、必要になったらその都度/docs/内のファイルを確認する
    - 確認する際は, /agentsの1つであるknowledge-researcherを用いて、そこから要約情報を受けとる
    - 外部情報を参照する必要が出た際も、knowledge-researcherエージェントを用いて調査をして、そのエージェントから要約情報を受け取る
- コーディングの際、プロジェクトによってはFDDの原則を全て必ずしも守らなくて良い(src/ではできる限りFDDの原則を使用)
- 実装作業フェーズのどこまで終了したかを細かいタスクが完了するごとに逐次報告する
    - 各jlファイル内のどの部分の実装が終わったかを✅で報告する(例: src/Domain/Types.jlのどの部分が終わったかを✅で報告する)
- errorとその訂正情報は/docs/error_log.mdに記載する

## Test

- 全テスト: `julia --project -e 'using Pkg; Pkg.test()'`
- 個別実行: `julia --project -e 'using Test, HONetSync; @testset "名前" begin include("test/対象ファイル.jl") end'`
